	...
	<div id="akeeba-buadmin-download-{{ (int)$record['id'] }}" tabindex="-2" role="dialog">
        <div class="akeeba-renderer-fef">
	        ...
	        @if($record['multipart'] < 2)
                <a class="akeeba-btn--primary--small" href="javascript:confirmDownload('{{ $thisID }}', '');">
	        ...
	        @if($record['multipart'] >= 2)
		        ...
		        @for($count = 0; $count < $record['multipart']; $count++)
			        @if($count > 0)
                        &bull;
			        @endif
                    <a class="akeeba-btn--small--dark" href="javascript:confirmDownload('{{ $thisID }}', '{{ urlencode($count) }}');">
...
<div style="padding-bottom: 3pt;">
	<a class="akeeba-btn--primary akeeba_remote_management_link"
	   href="javascript:akeeba.Manage.onRemoteManagementClick('index.php?option=com_akeeba&view=RemoteFiles&tmpl=component&task=listactions&id={{ (int)$record['id'] }}','index.php?option=com_akeeba&view=Manage')"
	>
...
@elseif($showUploadRemote)
	<a class="akeeba-btn--primary akeeba_upload"
	   href="javascript:akeeba.Manage.onUploadClick('index.php?option=com_akeeba&view=Upload&tmpl=component&task=start&id={{ (int)$record['id'] }}','index.php?option=com_akeeba&view=Manage')"
